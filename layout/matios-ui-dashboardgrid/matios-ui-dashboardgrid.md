...
<!-- Optional: localized chrome (drag/remove/resize labels, tray title/empty).
     Requires base/matios-ui-i18n.js loaded first. Without it, defaults to Spanish. -->
<script src="layout/matios-ui-dashboardgrid/matios-ui-dashboardgrid-i18n.js"></script>
```

The optional widget HTML is sanitized with `MTS.Sanitize` when
`utilities/matios-ui-sanitize/matios-ui-sanitize.js` is loaded (otherwise the raw
`item.html` is injected as-is).
...
| `debug` | `boolean` | `false` | When `true`, logs each layout change to the console (a grouped `[MTS.DashboardGrid] onWidgetsChange` report) and logs any error thrown by `onItemMounted` |
| `items` | `Item[]` | `[]` | Active widgets in the grid |
| `availableItems` | `Item[]` | `[]` | Available widgets in the tray |
| `onWidgetsChange` | `function(layout)` | `null` | Callback with the updated layout `[{id, col, row, w, h}]`. Fired on every commit: drag-move, resize, drop from tray, remove (`×`), keyboard move, and the `addItem()` / `removeItem()` methods |
...
The only text the grid renders itself is the edit-mode chrome: the drag-handle,
remove and resize control labels (`aria-label` + `title`), the tray title, and
the tray empty-state. Everything else (widget titles and `html`) is
consumer-supplied and is never translated by the grid.

This chrome is localized through the global i18n API under the `MTS.DashboardGrid`
namespace. Load `matios-ui-dashboardgrid-i18n.js` (after `base/matios-ui-i18n.js`)
and set the language once at startup:
...
| Key | Spanish (default) | English | Portuguese |
|---|---|---|---|
| `dragHandle` | Mover widget | Move widget | Mover widget |
| `removeWidget` | Eliminar widget | Remove widget | Remover widget |
| `resizeHandle` | Redimensionar widget | Resize widget | Redimensionar widget |
| `trayTitle` | Widgets disponibles | Available widgets | Widgets disponíveis |
| `trayEmpty` | Todos los widgets están en el dashboard | All widgets are on the dashboard | Todos os widgets estão no dashboard |

The `×` glyph on the remove button is a bare character with no text; only its
`aria-label` / `title` are localized.
