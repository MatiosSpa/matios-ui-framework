...
MTS.JsonViewer.prototype._renderBody = function() {
  ...
  if (this._parseError) {
    ...
    let rawEl = document.createElement('pre');
...
MTS.JsonViewer.prototype._renderValue = function(value, path, level, key, isRoot) {
  ...
  let collapsed = this._collapsed.has(path);
  ...
  if (collapsed) {
    ...
    let closeCollapsed = document.createElement('span');
  ...
  if (!collapsed) {
    ...
    if (!keys.length) {
      ...
      keys.forEach(function(childKey) {
        ...
        let childPath = isArray ? path + '[' + childKey + ']' : path + '.' + childKey;
