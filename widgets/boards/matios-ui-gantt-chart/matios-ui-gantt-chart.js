...
(function (global) {
  ...
  function startOfDay(ts)   { let d = new Date(ts); d.setHours(0,0,0,0); return d.getTime(); }
  function startOfWeek(ts)  { let d = new Date(ts); d.setDate(d.getDate() - (d.getDay() + 6) % 7); d.setHours(0,0,0,0); return d.getTime(); }
  function startOfMonth(ts) { let d = new Date(ts); d.setDate(1); d.setHours(0,0,0,0); return d.getTime(); }

  function addDays(ts, n)   { return ts + n * MS_DAY; }
  function addWeeks(ts, n)  { return addDays(ts, n * 7); }
  function addMonths(ts, n) { let d = new Date(ts); d.setMonth(d.getMonth() + n); return d.getTime(); }
  ...
  function flattenTree(tasks, level) {
    ...
    for (let i = 0; i < tasks.length; i++) {
      ...
      result.push(t);
      if (t._hasChildren) {
        let sub = flattenTree(t.children, level + 1);
  ...
  function normalizeTasks(tasks, hoursPerDay) {
    let flat;
    let hasChildren = tasks.some(function (t) { return t.children && t.children.length; });
    if (hasChildren) {
  ...
  class MtsGanttChart {
    ...
    updateTask(id, fields) {
      let task = this._taskById(id);
      if (!task) return this;
      this._pushHistory();
      let keys = Object.keys(fields);
      for (let i = 0; i < keys.length; i++) task[keys[i]] = fields[keys[i]];
      this._recalc(task);
    ...
    _normalizeCanonicalTask(t) {
      ...
      let deps = t.predecessors != null ? t.predecessors : t.deps;
      ...
      if (t.wbs) {
        ...
        this._tasks.forEach(function (x) {
          let w = String(x.wbs);
          if (w.indexOf('.') < 0) { let n = parseInt(w, 10); if (!isNaN(n) && n > maxTop) maxTop = n; }
        });
        out.wbs = String(maxTop + 1);
      }

      // arrastra campos extra que el dev haya cosechado (round-trip / backend)
      let known = { id:1, label:1, name:1, title:1, start:1, Start:1, end:1, finish:1, Finish:1,
    ...
    submitAddTask() {
      ...
      function finish(task) {
        if (settled) return;
        settled = true;
        let t = (task && typeof task === 'object') ? task : data;
    ...
    exportTasks(format, filename) {
      format = format || 'csv';
      let ext = format === 'csv' ? 'csv' : (format === 'excel' ? 'xlsx' : (format === 'msproject' ? 'xml' : format));
      let tasks   = this.getTasks();
      let payload = { format: format, filename: filename || ('proyecto.' + ext), tasks: tasks };
    ...
    importFile(file) {
      if (!file) return this;
      let name = file.name || '';
      let ext  = name.indexOf('.') >= 0 ? name.split('.').pop().toLowerCase() : '';
      let format = ext === 'csv' ? 'csv'
    ...
    _buildCSV(tasks) {
      ...
      let header = cols.map(function (c) { return cell(c.label); }).join(',');
      let rows = tasks.map(function (t) {
    ...
    _clearDropMarks() {
      ...
      let marked = this._gridBody.querySelectorAll('.mts-gantt__tr--drop-before, .mts-gantt__tr--drop-after, .mts-gantt__tr--drop-into');
      for (let i = 0; i < marked.length; i++) this._unmarkDrop(marked[i]);
    }

    _buildTree() {
      let sorted = this._tasks.slice().sort(function (a, b) { return wbsCompare(a.wbs, b.wbs); });
      let map = {}, roots = [];
      sorted.forEach(function (t) {
        let node = { task: t, children: [] };
        let wbs  = String(t.wbs);
        map[wbs] = node;
        let dot  = wbs.lastIndexOf('.');
        let parentWbs = dot >= 0 ? wbs.substring(0, dot) : null;
    ...
    _findNode(nodes, id) {
      for (let i = 0; i < nodes.length; i++) {
        ...
        let found = this._findNode(nodes[i].children, id);
    ...
    _renumber(nodes, prefix) {
      for (let i = 0; i < nodes.length; i++) {
        let wbs = prefix ? prefix + '.' + (i + 1) : String(i + 1);
    ...
    _moveTask(dragId, targetId, mode) {
      ...
      let roots    = this._buildTree();
      let dragInfo = this._findNode(roots, dragId);
      ...
      let targetInfo = this._findNode(roots, targetId);
      if (!targetInfo) return this;
      if (mode === 'into') {
        targetInfo.node.children.push(dragInfo.node);
      } else {
        let arr = targetInfo.parentArr;
        let idx = arr.indexOf(targetInfo.node);
    ...
    deleteTask(id) {
      let task = this._taskById(id);
      if (!task) return this;
      let wbsStr = String(task.wbs);
      let self = this;
      let toRemove = this._tasks.filter(function (t) {
        return t.id === id || String(t.wbs).startsWith(wbsStr + '.');
      });
      this._pushHistory();
      toRemove.forEach(function (t) {
        let i = self._tasks.indexOf(t);
    ...
    removeLink(id) {
      let idx = this._links.findIndex(function (l) { return l.id === id; });
      if (idx < 0) return this;
      let link = this._links.splice(idx, 1)[0];
    ...
    _snapshot() {
      let clean = this._tasks.map(function (t) {
        let c = {};
        for (let k in t) { if (t.hasOwnProperty(k) && k.charAt(0) !== '_') c[k] = t[k]; }
        return c;
      });
    ...
    getCode() {
      let sel = (this._el && this._el.id) ? ("'#" + this._el.id + "'") : "'#gantt'";
      return [
    ...
    _load() {
      ...
      let query = this._opts.queryParams || {};
      let promise;
    ...
    _buildDOM() {
      ...
      // Panel derecho
      let ganttPanel = document.createElement('div');
      ganttPanel.className = 'mts-gantt__gantt-panel';

      let ganttHeader = document.createElement('div');
      ganttHeader.className = 'mts-gantt__gantt-header';

      let ganttBody = document.createElement('div');
    ...
    _getColWidths() {
      let cols   = this._visibleColumns();
      let panelW = this._gridPanel ? this._gridPanel.clientWidth : 0;
      let sbW    = (this._gridBody && this._gridBody.offsetWidth > 0)
                    ? (this._gridBody.offsetWidth - this._gridBody.clientWidth)
                    : 0;
      let available = panelW - sbW;
    ...
    _buildGridHeader(container, widths) {
      ...
      let cols = this._visibleColumns();
      let w = widths || this._getColWidths();
      for (let i = 0; i < cols.length; i++) {
        let col = cols[i];
    ...
    _renderGrid() {
      let self   = this;
      let widths = this._getColWidths();
      let totalW = widths.reduce(function (a, b) { return a + b; }, 0);
      this._buildGridHeader(this._gridHeader, widths);
      this._gridBody.innerHTML = ''; // safe: clearing
      let rows = this._visibleRows();
      if (!rows.length) { this._showEmptyGrid(); return; }
      for (let ri = 0; ri < rows.length; ri++) {
        let row = this._buildGridRow(rows[ri], widths);
    ...
    _buildCell(task, col, colWidth) {
      ...
      let field = col.field;
      let w     = colWidth != null ? colWidth : (col.width || 80);
      ...
      if (field === 'label') {
        ...
        if (task._hasChildren) {
          ...
          let svg  = svgEl('svg', { viewBox: '0 0 10 10', fill: 'none', width: 10, height: 10 });
          let path = svgEl('path', { d: 'M3 1l4 4-4 4', stroke: 'currentColor', 'stroke-width': '1.5', 'stroke-linecap': 'round', 'stroke-linejoin': 'round' });
        ...
        let pctVal = Math.round((task.progress || 0) * 100);
        ...
        let pctNum = document.createElement('span');
        ...
        if (task._baseEndTs && task._endTs) {
          let dd  = Math.round((task._endTs - task._baseEndTs) / MS_DAY);
          let vsp = document.createElement('span');
        ...
        if (col.editable !== false && this._editable && global.MTS && global.MTS.ColorPicker) {
          let cpWrap = document.createElement('div');
        ...
        for (let ai = 0; ai < Math.min(assignees.length, 3); ai++) {
        ...
        let hasAssigneesListener = this._listeners.assigneesClick && this._listeners.assigneesClick.length;
      ...
      let notEditable = !col.editable || !this._editable ||
        field === 'wbs' || field === 'duration' || field === 'durationHours';

      let hasCellEditListener = this._listeners.cellEdit && this._listeners.cellEdit.length;
    ...
    _startEdit(task, col, td) {
      ...
      let oldValue = field === 'predecessors' ? (task.predecessors || []).slice() : task[field];
    ...
    _renderGantt() {
      ...
      let xEnd   = addScale(cols[cols.length - 1], 1, scale);
      let xRange = xEnd - cols[0];
      let contW  = this._ganttPanel.clientWidth || 600;
      let totalW = Math.max(contW, cols.length * 60);
      let colW   = totalW / cols.length;

      function xScale(t) { return (t - cols[0]) / xRange * totalW; }

      // ── Header SVG ────────────────────────────────────────

      let hSvg = svgEl('svg', { width: totalW, height: HEADER_H, style: 'display:block' });
      hSvg.appendChild(svgEl('rect', { x: 0, y: 0, width: totalW, height: HEADER_H, fill: 'var(--mts-bg-surface-2)' }));

      let MTHS  = this._t('months', MONTHS);
      let prevM = -1, mStart = 0;
      function flushMonth(endCi) {
        if (prevM < 0) return;
        let mx0 = xScale(cols[mStart]);
        let mx1 = endCi < cols.length ? xScale(cols[endCi]) : xScale(xEnd);
        let mw  = mx1 - mx0;
        if (mw < 4) return;
        hSvg.appendChild(svgEl('line', { x1: mx0.toFixed(1), y1: 0, x2: mx0.toFixed(1), y2: MONTH_H, stroke: 'var(--mts-border-color)', 'stroke-width': 1 }));
        let ml = svgEl('text', { x: (mx0 + mw / 2).toFixed(1), y: (MONTH_H / 2).toFixed(1), 'text-anchor': 'middle', 'dominant-baseline': 'middle', 'class': 'mts-gantt__axis-label mts-gantt__axis-label--month' });
        ml.textContent = MTHS[new Date(cols[mStart]).getMonth()] + ' ' + new Date(cols[mStart]).getFullYear();
        hSvg.appendChild(ml);
      }
      for (let ci = 0; ci < cols.length; ci++) {
        let m = new Date(cols[ci]).getMonth();
      ...
      for (let ci = 0; ci < cols.length; ci++) {
        let cx = xScale(cols[ci]);
        let cw = ci < cols.length - 1 ? xScale(cols[ci + 1]) - cx : colW;
        if (cw < 14) continue;
        let sl = svgEl('text', { x: (cx + cw / 2).toFixed(1), y: (MONTH_H + SCALE_H / 2).toFixed(1), 'text-anchor': 'middle', 'dominant-baseline': 'middle', 'class': 'mts-gantt__axis-label' });
        let cd = new Date(cols[ci]);
      ...
      let chartH = rows.length * rowH;
      let bSvg   = svgEl('svg', { width: totalW, height: chartH, style: 'display:block' });

      let defs  = svgEl('defs');
      let mkr   = svgEl('marker', { id: 'mts-gantt-arrow', markerWidth: 7, markerHeight: 7, refX: 6, refY: 3.5, orient: 'auto' });
      ...
      for (let ri = 0; ri < rows.length; ri++) {
        let task  = rows[ri];
        let rowY  = ri * rowH;
        let color = task.color || PALETTE[pIdx % PALETTE.length];
        if (!task.color) pIdx++;

        if (task._duration === 0) {
          let mx = xScale(task._startTs);
          let my = rowY + rowH / 2;
          let ms = barH / 2;
        ...
        let bx   = xScale(task._startTs);
        let bw   = Math.max(4, xScale(task._endTs) - bx);
        let barY = rowY + barPad;
        ...
        if (task._baseStartTs && task._baseEndTs) {
          let blx = xScale(task._baseStartTs);
          let blw = Math.max(2, xScale(task._baseEndTs) - blx);
          let blRect = svgEl('rect', {
            x: blx.toFixed(1), y: (barY + barH + 1).toFixed(1), width: blw.toFixed(1), height: 4,
            rx: 2, 'class': 'mts-gantt__bar-baseline',
          });
          let blTitle = svgEl('title');
        ...
        let ilbl = svgEl('text', { x: (bx + bw / 2).toFixed(1), y: (barY + barH / 2).toFixed(1), 'class': 'mts-gantt__bar-label', opacity: bw > 50 ? '1' : '0' });
    ...
    _renderDeps(gDeps, barMeta, xScaleFn, rows) {
      ...
      let wbsMap = {};
      for (let i = 0; i < this._tasks.length; i++) wbsMap[this._tasks[i].wbs] = this._tasks[i];

      for (let ri = 0; ri < rows.length; ri++) {
        let task = rows[ri];
        if (!task.predecessors || !task.predecessors.length) continue;
        let tgt = barMeta[task.id];
        if (!tgt || !tgt.barEl) continue;

        for (let pi = 0; pi < task.predecessors.length; pi++) {
          let pred = wbsMap[task.predecessors[pi]];
          if (!pred) continue;
          let src = barMeta[pred.id];
          if (!src || !src.barEl) continue;

          let sx  = src.bx + src.bw;
          let sy  = src.barY + src.barH / 2;
          let tx  = tgt.bx;
          let ty  = tgt.barY + tgt.barH / 2;
          let gap = tx - sx;
          let cp1 = sx + Math.max(16, gap * 0.4);
          let cp2 = tx - Math.max(16, gap * 0.4);
    ...
    _setupDrag(svg, barMeta, xScaleFn, xRange, totalW, rowH, scale) {
      ...
      function updateBarEl(meta) {
        let task = meta.task;
        let bx   = xScaleFn(task._startTs);
        let bw   = Math.max(4, xScaleFn(task._endTs) - bx);
      ...
      function onMouseMove(e) {
        ...
        e.preventDefault();
        let dx   = svgPx(e) - ds.startPx;
        let dt   = dx / totalW * xRange;
        let task = ds.meta.task;

        if (ds.type === 'move') {
          let dur = ds.origEnd - ds.origStart;
          let ns  = snapToScale(ds.origStart + dt, scale);
          task._startTs = ns;
          task._endTs   = ns + dur;
        } else if (ds.type === 'resize-right') {
          let ne = snapToScale(ds.origEnd + dt, scale);
      ...
      function onMouseUp() {
        ...
        let eventName = ds.type === 'move' ? 'taskMove' : 'taskResize';
    ...
    _updateGridRowDates(task) {
      let row = this._gridBody.querySelector('[data-task-id="' + task.id + '"]');
      if (!row) return;
      let tds  = row.querySelectorAll('.mts-gantt__td');
      let cols = this._visibleColumns();
      for (let i = 0; i < cols.length; i++) {
        let col = cols[i];
        let td  = tds[i];
    ...
    _showLoading() {
      ...
      for (let i = 0; i < 8; i++) {
        ...
        let widths = [40, 140, 70, 70, 40, 40, 60];
